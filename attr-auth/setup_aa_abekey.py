...
def Deserialize_user_CA():
    user_pub_CA = {}
    jsUP = {}
    if os.path.isfile("User_publickey.json"):
        with open("User_publickey.json", "r") as f:
            jsUP = json.load(f)
        f.close()
               
    
        user_pub_CA ['uid'] = jsUP['uid']
        user_pub_CA['uPK'] = {'pk': groupObj.deserialize(jsUP['uPK']['pk'].encode()),
                              'sk': groupObj.deserialize(jsUP['uPK']['sk'].encode()) 
                        }
    print("user_pub_CA: ", user_pub_CA)
...
if __name__ == "__main__":
    ...
    authorities['id'] = aid   # thuc te nhan tu CA    

    authorityAttributes = ['HEAD-DIRECTOR', 'DIVISIONAL MANAGEMENT', 'CEO', 'CTO', 'CIO', 'BRANCH MANANGER', 'ASSISTANT OF BM', 'CUSTOMER SERVICE', 'RELATIONSHIP MANAGEMENT', 'CONSULTANT', 'CENTRAL', 'PROV1', 'PROV2', 'PROV3', 'PROV4', 'PROV5'] # cho nay confirm lai, chua nghi ra tiep

    authority = aid #"authority1"  # em dang can nhac de cho nay la attrib['id'] luon
    if os.path.isfile("global-params.json"):   # trien khai thi gui gui tu CA qua 
        ...
        SKa, PKa, authAttrs = maabe.setupAuthority(GPP, authority, authorityAttributes, authorities)

        if not os.path.isfile("aa-publickeyabe.json"): #gửi qua cho owner cho nó chạy hàm encrypt
            #print("hello:")
            #PK = {}
            PK = {
            ...
            jsAuthority = {
                'PK': ({}, PK, PK_do),
                'authr':authorityAttributes
            }
            # print("JSONAU: ",jsAuthority)

            with open ("aa-publickeyabe.json", "w") as f:
                json.dump(jsAuthority, f)
            f.close()
    

        if not os.path.isfile("aa-privatekey.json"):
            jsUSK = {
    ...
    jsLog = {
        'id': 'attr-auth',
        'log': 'Setup attribute authority ABE keys'
    }
