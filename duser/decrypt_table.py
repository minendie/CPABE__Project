...
def Register_User(GPP,alice):
    maabe = MAABE(groupObj)
    #alice = { 'id': 'alice', 'authoritySecretKeys': {}, 'keys': None, 'set_of_uspk':{} }
    
    #register
    alice['keys'], alice['set_of_uspk'] = maabe.registerUser(GPP) #private , public
    print(alice['set_of_uspk'])
    
    str_sk = groupObj.serialize(alice['set_of_uspk']['sk'])
    str_pk = groupObj.serialize(alice['set_of_uspk']['pk'])
    send_user= {
        'alice': {'pk': str_pk, 'sk': str_sk}
    }

    str_send_user = str(send_user)
    print(str_send_user)
...
def Deserialize_User_Skey_AA(uSK_aa):
    ...
    AK = {}
    for attrib in authorityAttributes: #[0:len(authorityAttributes)]:
...
def get_key(uid):
    ...
    # querry abe public key from AA neu node nay la owner (update: owner duoc quy ve thang bank-server-app)
    '''
    ...
    CT = maabe.encrypt(GPP, policy_str, k, authority_enc)

    print("CT: ", CT)
    '''


    r = remote("attr-auth", 2001, ssl=True)    # request SK de decrypt

    r.sendlineafter(b"uid: ", uid.encode())

    with open(f"user-info-{uid}.json") as f:
        user_info = f.read()
    f.close()

    r.sendlineafter(b"user_info: ", user_info.encode())
    jsU = json.loads(user_info)

    user_secret_abe = r.recvline().strip().decode()

    jsUSK = json.loads(user_secret_abe)
    user_attributes = jsU['attributes']

    USK = Deserialize_USK(jsUSK, user_attributes)
    #dec_userkey = {}
    dec_userkey['uid'] = uid # user1_private_CA['uid']
    dec_userkey['authoritySecretKeys'] = USK
    dec_userkey['keys'] = (user1_private_CA['uSK'][0],user1_private_CA['uSK'][1])
    # print("dec_userkey: ", dec_userkey)
    # print ("k: ", k)
    # PT1 = maabe.decrypt(GPP, CT, dec_userkey)
    # print ("PT1: ", PT1)
    #jsAuth = json.loads(r.recvline().strip().decode())
    #authority_dec = Deserialize_User_Skey_AA(jsAuth)

    r.close()


from aes_gcm_128_helpers import decrypt_AES_GCM
from cp_abe_services import deserialize_GPP, deserialize_ciphertext
...
def encrypt_datarow(cursor, str_querry):   #cái này gửi lên cloud 
    ...
    for row in rows:
        ...
        for index in range(len(row)):
            if(index == len(row)-1):
                plaintext += str(row[index])
            else:
                plaintext += str(row[index])+'\x00'
        '''

        plaintext = str(row).encode()
        ...
        policy_str = '((HEAD-DIRECTOR or CIO) and (CENTRAL or PROV5))'
        #send key <pairing group> to Encrypt(content key)
        encrypted_key = encrypt_content_key(key, policy_str)
        ret_ciphertext = encrypt_AES_GCM(plaintext, key_AES) # <byte>
        #convert ret_cipher to base64 from bytes
        ...
        my_tuple= (b64_ret_ciphertext, b64_encrypted_key) # (ciphertext, enc(content k) )
        enc_data.append(my_tuple)

    return enc_data


def decrypt_datarow(cursor, str_querry):
    cursor.execute(str_querry)
    rows = cursor.fetchall()
    dec_data = []
    ...
    for row in rows:
        my_tuple = []
        key = decrypt_content_key(row[len(row)-1])
        key_AES = groupObj.serialize(key)[:16]
        for i in range(len(row)-1):
            ciphertext = base64.b64decode(row[i].encode())
            plaintext = decrypt_AES_GCM(ciphertext, key_AES)
            my_tuple+= (plaintext.decode(),)
        
        dec_data.append(my_tuple)
...
def decrypt_content_key(encrypted_key):
    # key pairing group GT
    maabe = MAABE(groupObj=groupObj)
    

    encrypted_key = base64.b64decode(encrypted_key.encode()).decode()
    encrypted_key = deserialize_ciphertext(encrypted_key)
    ...
    if os.path.isfile("global-params.json"): #nhan tu CA
        ...
        f.close()
    GPP = deserialize_GPP(jsGPP)
    k = maabe.decrypt(GPP, encrypted_key, dec_userkey)
    #print("k decrypted", k)
...
try:
    ...
    dec_data = decrypt_datarow(cursor_cloud, "SELECT * FROM Bank")
